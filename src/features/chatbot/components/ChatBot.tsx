...
import type {ChatButtonOption, UserData } from '@/lib/chatFlowTypes';
...
export default function ChatBotInternal() {
  ...
  const handleUserResponse = (value: string, isButtonClick = false) => {
    ...
    startTransition(async () => {
        ...
        let userDataAfterAction = { ...useChatStore.getState().userData };
        
        if (isButtonClick) {
            const stepOptions = typeof currentStep.options === 'function' ? currentStep.options(userDataAfterAction) : currentStep.options;
            ...
            if (selectedOption) {
                ...
                if (currentStep.action && !currentStep.isUserInput) {
                    const dataFromAction = await currentStep.action(selectedOption.value || selectedOption.label, userDataAfterAction);
                    userDataAfterAction = { ...userDataAfterAction, ...dataFromAction };
                }
            }
        } else { 
            if (currentStep.isUserInput) {
                if (currentStep.action) {
                    const dataFromAction = await currentStep.action(value, userDataAfterAction);
                    userDataAfterAction = { ...userDataAfterAction, ...dataFromAction };
                }
        ...
        setUserDataInStore(userDataAfterAction);

        if (nextStepIdToGo) {
            const nextStep = chatFlow[nextStepIdToGo];
            const finalUserDataForAction = useChatStore.getState().userData;
            
            if (nextStep && nextStep.action && nextStep.endFlow) {
                const finalDataUpdate = await nextStep.action("", finalUserDataForAction); 
                setUserDataInStore(finalDataUpdate);
            }
            
            const navigateToPath = typeof nextStep?.navigateTo === 'function' ? nextStep.navigateTo(finalUserDataForAction) : nextStep?.navigateTo;
            if(navigateToPath) {
            ...
            processStep(nextStepIdToGo, useChatStore.getState().userData);
        }
    });
  };
  ...
  const isSubmitDisabled = 
    ...
    currentStepDef.validation("", userData) !== null;
              ...
              {m.sender === "bot" && m.buttons && m.buttons.length > 0 && (
                <div className="mt-3 flex flex-wrap gap-2">
                  {m.buttons.map((button, btnIdx) => ( 
                    <Button key={`${button.value || button.label}-${btnIdx}`} variant="default" size="sm" className="h-auto py-2 px-3 text-sm text-center bg-[#708BA0] hover:bg-[#5c7081] text-white border-0 flex-grow basis-[calc(50%-0.25rem)]" onClick={() => handleOptionClick(button)}>
                      {button.label}
                    </Button>
                  ))}
                </div>
      ...
      <CardFooter className="border-t p-3 sm:p-4 bg-gray-50 dark:bg-gray-800 flex-shrink-0 w-full flex flex-col items-start">
        ...
        {currentStepDef?.isUserInput ? (
          <form onSubmit={handleSubmitInput} className="flex w-full items-center space-x-2">
            {canGoBack && (<Button type="button" variant="ghost" size="icon" onClick={handleGoBack} aria-label="Atrás" className="flex-shrink-0"><ArrowLeft className="h-4 w-4" /></Button>)}
            <Input ref={inputRef} type={currentStepDef.inputType || "text"} value={userInput} onChange={(e) => setUserInput(e.target.value)} placeholder={currentStepDef.inputPlaceholder || "Escribe tu mensaje..."} className="flex-1 h-10 text-[16px] sm:text-sm min-w-0" aria-label="Escribe tu respuesta"/>
            <Button type="submit" size="icon" disabled={isPending || isSubmitDisabled} aria-label="Enviar respuesta" className="flex-shrink-0 bg-[#708BA0] hover:bg-[#5c7081]"><Send className="h-4 w-4" /></Button>
          </form>
        ) : (canGoBack && (<div className="w-full flex"><Button type="button" variant="outline" onClick={handleGoBack} aria-label="Atrás" className="gap-2"><ArrowLeft className="h-4 w-4" /> Atrás</Button></div>))}
      </CardFooter>
    </>
  );
}